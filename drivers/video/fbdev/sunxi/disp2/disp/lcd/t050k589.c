...
static struct LCM_setting_table rm68200_initialization_setting[] = {
...
static struct LCM_setting_table LCM_S055SWA3_setting[] = {
{0xE0,1,{0x00}},
{0xE1,1,{0x93}},
{0xE2,1,{0x65}},
{0xE3,1,{0xF8}},
...
static void lcd_panel_init(u32 sel)
{
	...
	//sunxi_lcd_delay_ms(120);
#if 0
	DBG_INFO("initialization:LCM_S055SWA3\n");
	for(index = 0;; ++index){
		DBG_INFO("index:%d, count:%d, cmd:%d, para_lsit[0]:%d.\n",
				  index,
				  LCM_S055SWA3_setting[index].count,
				  LCM_S055SWA3_setting[index].cmd,
				  LCM_S055SWA3_setting[index].para_list[0]);

		if(LCM_S055SWA3_setting[index].count == REGFLAG_END_OF_TABLE){
			break;
		}else if (LCM_S055SWA3_setting[index].count == REGFLAG_DELAY){
			sunxi_lcd_delay_ms(LCM_S055SWA3_setting[index].para_list[0]);
		}else{
			sunxi_lcd_dsi_dcs_write(sel, LCM_S055SWA3_setting[index].cmd, 
						    LCM_S055SWA3_setting[index].para_list,
							LCM_S055SWA3_setting[index].count);
		}
	}
#endif	
...
#if 0
	DBG_INFO("initialization:rm68200\n");
	for(index = 0;; ++index){
		//DBG_INFO("index:%d, count:%d, cmd:%d, para_lsit[0]:%d.\n",
		//		  index,
		//		  rm68200_initialization_setting[index].count,
		//		  rm68200_initialization_setting[index].cmd,
		//		  rm68200_initialization_setting[index].para_list[0]);

		if(rm68200_initialization_setting[index].count == REGFLAG_END_OF_TABLE){
			break;
		}else if (rm68200_initialization_setting[index].count == REGFLAG_DELAY){
			sunxi_lcd_delay_ms(rm68200_initialization_setting[index].para_list[0]);
		}else{
			sunxi_lcd_dsi_dcs_write(sel, rm68200_initialization_setting[index].cmd, 
						    rm68200_initialization_setting[index].para_list,
							rm68200_initialization_setting[index].count);
		}
	}	
#endif
